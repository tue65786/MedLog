...
public class HealthcareProviderVO implements Serializable, IEntityBase<HealthcareProviderVO> {
    ...
    /**
     * @param patientLogCommunicationPreference the
     * patientLogCommunicationPreference to set
     */
        ...
        if (_ACTION == INSERT) {
            ...
            if (getAddressStateID() == null) {
                ...
                if (this.getPhysicianID() <= 0) {
                    LOG.severe("Healthcare provider error: PhysicianID not specified");
                    return false;
                }
            }
        }
        boolean reqP = !(getLastName() == null || getFirstName() == null || getLastName().length() * getFirstName().length() == 0);
        if (!reqP) {
            LOG.severe("Healtcareprovider not valid. First/Last name are required.");
        }
        return reqP;
    }
