...
public class MedicationVO implements Serializable, IEntityBase<MedicationVO>, Comparable<MedicationVO> {
    ...
    public String toTableRow() {
       final  String style = "border-bottom:1px solid blue;";
       
        return "<tr style='" + style + "border-top:1px solid blue;border-radius:3px;'>"
                + "<td style='" + style + "'>" + this.getPharmID().getFullName() + "</td>"
                + "<td style='" + style + "'> " + StrUtl.getDateWithFormat(this.getStartDate()) + "</td><td style='" + style + "'>" + this.getSig().getSigAbbrID() + "</td><td style='" + style + "'>" + this.getInstructions() + "</td>"
                + "<td style='border-bottom:1px solid blue;'>"
                + StrUtl.toS(this.getPhysicianID().getLastName() + ", " + this.getPhysicianID().getFirstName(), "N/A")
                + "</td></tr>";
//    throw new UnsupportedOperationException( "Not supported yet." ); //To change body of generated methods, choose Tools | Templates.
    }
